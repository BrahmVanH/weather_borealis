...
import { SET_WEATHER_LOCATION, SET_CURRENT_WEATHER, SET_FORECAST_WEATHER, WEATHER_LOCATION_LOADED } from '../../utils/actions';
		...
		<Nav id='mainNav' className='navbar navbar-light navbar-expand-md'>
			<div className='container'>
				...
				<Form className='navbar-brand d-flex align-items-center' onSubmit={handleSearchInput}>
					...
					<Form.Group>
						<Form.Control style={{ userSelect: 'all' }} type='text' size='sm' placeholder='city or zip' name='userSearchInput' onChange={handleInputChange} value={userSearchInput.userSearchInput} />
						<Form.Control.Feedback type='invalid'>Location is required for search</Form.Control.Feedback>
					</Form.Group>
					<Button type='submit' className='search-button' variant='light' size='sm'>
						<FaSearch />
					</Button>
				</Form>
				<Dropdown id='navbarResponsive' className='collapse navbar-collapse' style={{ transform: 'translate(-191)' }}>
					<section className='py-4 py-xl-5'></section>
					<i className='fas fa-search'></i>
					<ul className='navbar-nav ms-auto'>
						<li className='nav-item nav-link'>
							<Link className={`nav-link ${activeTab === '/' ? 'active' : ''}`} to='/'>
								Home
							</Link>
						</li>
						<li className='nav-item nav-link'>
							<Link className={`nav-link ${activeTab === '/aurora' ? 'active' : ''}`} to='/aurora'>
								Aurora Forecast
							</Link>
						</li>
						<li className='nav-item nav-link'>
							<Link className={`nav-link ${activeTab === '/contact' ? 'active' : ''}`} to='/contact'>
								Contact
							</Link>
						</li>
					</ul>
